---
name: use-modern-go
description: Apply modern Go syntax and standard-library guidance that is compatible with the active module's Go version. Use when writing, reviewing, or modernizing Go code where version-aware language choices matter.
---
...
Before writing or reviewing Go code, read the `go` directive from the `go.mod` governing the package being changed. For this single-module project, use the root `go.mod`. In a future multi-module project, use the nearest enclosing `go.mod`.

If no applicable `go.mod` exists and the requested change depends on version-specific features, ask the user which Go version to target. Do not infer support from the locally installed toolchain because it may be newer than the project's compatibility target.

**When writing Go code**, use features from this document only up to the target version:
